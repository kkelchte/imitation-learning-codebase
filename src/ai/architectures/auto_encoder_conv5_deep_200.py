    ...
    def __init__(self, config: ArchitectureConfig, quiet: bool = False):
        ...
        if not quiet:
            self.encoder = nn.Sequential(
                nn.Conv2d(3, self.h, 5, stride=1, padding=2),
                *[nn.BatchNorm2d(self.h), nn.LeakyReLU()] if self._config.batch_normalisation
                else [nn.LeakyReLU()],
                nn.Conv2d(self.h, self.h * (2 if self.vae else 1), 5, stride=1, padding=2),
                *[nn.BatchNorm2d(self.h * (2 if self.vae else 1)), nn.LeakyReLU()] if self._config.batch_normalisation
                else [nn.LeakyReLU()],
            )
            self.decoder = nn.Sequential(
                nn.Conv2d(self.h, self.h, 5, stride=1, padding=2),
                nn.Conv2d(self.h, 1, 5, stride=1, padding=2),
            )
