...
class YoloLayer(nn.Module):
    ...
    def get_region_boxes(self, output, conf_thresh):
        ...
        det_confs = det_confs.to('cuda') #, non_blocking=True for torch 4.1?
        cls_max_confs = cls_max_confs.to('cuda')
        cls_max_ids = cls_max_ids.to('cuda')
        xs, ys = xs.to('cuda'), ys.to('cuda')
        ws, hs = ws.to('cuda'), hs.to('cuda')
        ...
        for ind in inds[det_confs > conf_thresh]:
            ...
            batch = math.ceil(ind.item()/(nA*nH*nW)) - 1
